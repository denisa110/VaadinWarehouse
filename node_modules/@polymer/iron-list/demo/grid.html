...
.photoContent>iron-image { @apply --layout-flex;
...
::-ms-input-placeholder {
	color: rgba(255, 255, 255, 0.5);
		...
		<iron-ajax id="ajax" loading="{{loadingPhotos}}"
			url="[[_getAPIEndpoint(apiKey, searchText, page)]]" handle-as="text"
			on-response="_didReceiveResponse"></iron-ajax>

		<app-header fixed effects="waterfall"> <app-toolbar>
		<input class="searchInput" type="search" placeholder="Search on Flikr"
			value="{{searchText::input}}"> </app-toolbar> </app-header>
		...
		<iron-scroll-threshold id="scrollTheshold" lower-threshold="500"
			on-lower-threshold="_onLowerThreshold" scroll-target="document">
