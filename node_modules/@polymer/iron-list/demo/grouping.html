...
.group-header, .group-item {
	padding: 15px 20px;
	background-color: white;
	font-size: 15px;
}

.group-item { @apply --layout-horizontal;
	cursor: pointer;
...
.state-list {
	...
	color: black;
		...
		<paper-menu-button id="menu" dynamic-align> <paper-icon-button
			icon="filter-list" class="dropdown-trigger"></paper-icon-button> <iron-list
			class="state-list dropdown-content" items="[[states]]">
		<template>
			<paper-item on-click="_onStateClick">[[item.state]]</paper-item>
		</template>
		</iron-list> </paper-menu-button> </app-toolbar>

		<iron-list id="list" items="[[_generateGroupsFor(items)]]"
