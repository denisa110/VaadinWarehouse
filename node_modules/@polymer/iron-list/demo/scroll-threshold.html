...
html, body { @apply --paper-font-common-base;
	margin: 0;
	padding: 0;
...
iron-list {
	padding-top: 64px;
	padding-bottom: 16px; -
	-iron-list-items-container: { max-width : 800px;
	margin: auto;
	margin-top: 60px;
	margin-bottom: 10px;
}
...
.personItem { @apply --layout-horizontal;
...
.index {
	width: 30px;
		...
		<iron-ajax id="ajax" url="https://randomuser.me/api/"
			params='{"results": 20}' handle-as="json" loading="{{loadingPeople}}"
		...
		<template>
			<div>
				<div class="personItem" tabindex$="[[tabIndex]]">
					<iron-image class="avatar" sizing="contain"
						src="[[person.picture.medium]]"></iron-image>
					<div class="pad">
						<div class="primary">[[person.name.first]]
		...
		<iron-scroll-threshold id="scrollTheshold" lower-threshold="500"
			on-lower-threshold="_load" scroll-target="document"></iron-scroll-threshold>
	</template>
	<script type="module">
