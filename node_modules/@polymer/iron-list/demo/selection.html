...
<body unresolved>
	...
	<template>
		<style>
:host {
	display: block; @ apply --paper-font-common-base;
	font-family: sans-serif;
}

.content { @apply --layout-vertical;
	height: 100%;
}
...
#itemsList, #selectedItemsList { @apply --layout-flex;
	
}

.item { @apply --layout-horizontal;
	cursor: pointer;
	padding: 16px 22px;
...
iron-list { @apply --layout-flex;
		...
		<div class="content">
			...
			<iron-list id="itemsList" items="[[data]]"
				selected-items="{{selectedItems}}" selection-enabled multi-selection>
			<template>
				<div>
		...
		<div class="content">
			...
			<div main-title>[[_getFormattedCount(selectedItems.length)]]
				Selected contacts</div>
			</app-toolbar>
			<template is="dom-if" if="[[!selectedItems.length]]">
				<div class="no-selection">Select a contact</div>
			</template>
			<!-- List for the selected items -->
			<iron-list id="selectedItemsList" items="[[selectedItems]]"
				hidden$="[[!selectedItems.length]]">
			<template>
				<paper-item tabindex$="[[tabIndex]]" on-tap="_unselect"
					aria-label$="Deselect [[item.name]]">[[item.name]]</paper-item>
			</template>
			</iron-list>
		</div>
		</app-drawer> </app-drawer-layout>
